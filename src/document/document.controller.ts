...
export class DocumentController {
    ...
    @Post("/")
    async createDocument(@Req() req:any,@Body() data:{parentDocument?:string}){
        return this.documentService.createDocument(data.parentDocument,req.user?.id)
    }
    ...
    @Put("/:id")
    async updateDocument(@Param("id",ParseUUIDPipe) document:any,@Body() data:updateDocument){
        return this.documentService.updateDocument(document,data);
    }


    @Put("/save/:id")
    async saveChange(@Param("id",ParseUUIDPipe) document:any,@Body() data:{content:any[]}){
        return this.documentService.setContent(document,data.content)
    }

    @Put("/rename/:id") 
    async rename(@Param("id",ParseUUIDPipe) document:any,@Body() data:{title:string}){
    ...
    @Post("/archieve/:id")
    async updateArchieve(@Param("id",ParseUUIDPipe) id,@Body() data:archieveDocumentDto){
